...
export function validateDelegatedToken(
  ...
  if (parts.length !== 4 || parts[0] !== DELEGATED_TOKEN_PREFIX || parts[1] !== DELEGATED_TOKEN_VERSION) {
    return null;
  }
  ...
  try {
    ...
    const signatureValid = verifySignature(
      ...
      Buffer.from(parts[2].length === 0 ? "" : parts[3], "base64url"),
    );
    if (!signatureValid) {
      return null;
    }

    const claims = JSON.parse(Buffer.from(parts[2], "base64url").toString("utf8")) as Partial<DelegatedTokenClaims>;
    if (
