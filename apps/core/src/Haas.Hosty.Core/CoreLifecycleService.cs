    ...
    MountPathPolicy? mountPathPolicy = null)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // the version that ran live in dev mode. Risk = a baseline was snapshotted at enable AND the app
        // has since run a different version (a likely one-way data migration the reviewed version may not
        // read back). When risky the app is left stopped and the caller is handed the pre-dev-mode backup
        // to offer for rollback before the reviewed version boots onto migrated data.
        ...
        // stop/start cycle to take effect. A non-selected runtime is not running — nothing to cycle.
        // Mirror the stop->operate->restart pattern of the manual-backup path so the enable snapshot
        // below copies stopped (consistent) data.
        var wasRunning = targetsSelected && manageLifecycle && string.Equals(app.RuntimeState, "running", StringComparison.Ordinal);
        if (wasRunning)
        {
