...
// One OTLP span parsed from the collector's file-exporter output, paired with the app it was
// attributed to via its `hosty.app.id` resource attribute (the same attribution the logs parser
// uses). Spans that carry no app id are dropped by the parser, never surfaced here.
...
// span missing its app attribution is skipped so one bad line cannot poison a tail. Spans without a
// trace id, span id, or start timestamp are dropped (they cannot be grouped into a trace or placed
// on the window). Low-level OTLP/JSON readers are shared with the logs parser (OtlpJsonParsing).
...
internal static class OtlpTracesJsonParser
{
    ...
    // `kind` is normally the integer SpanKind enum value; tolerate the enum name string too.
    // Normalized to a lowercase token so clients never parse OTLP enum names.
    ...
    // `status.code` is the integer StatusCode enum value or its name; the message only accompanies
    // an error status. Normalized to "unset"/"ok"/"error".
