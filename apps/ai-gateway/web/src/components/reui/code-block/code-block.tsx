...
function CodeBlock({
  ...
  }, [selection, selectedSet, selectedLines, onSelectedLinesChange])
  ...
  }, [foldedValue, foldedSet, folded, onFoldedChange])
...
function CodeBlockSurface({
  ...
  if (wasStreaming !== streaming) {
    ...
    setAnnouncement(streaming ? "" : completeAnnouncement ?? `Code generation complete, ${lines.length} lines.`)
  }
  ...
  const resolveScroller = useCallback((): HTMLElement | null => {
    ...
    while (node && node !== document.body) {
      // The conversation scroller owns its follow state, including a reader paused above.
      if (node.dataset.slot === "message-scroller-viewport") return viewport
  ...
  return (
    ...
    >
      ...
      <div
        ...
        id={contentId}
        role={selectable ? "listbox" : "region"}
...
function CodeBlockCopyButton({
  ...
  const handleClick = useCallback(() => {
    ...
    // An explicit copy value is the caller's exact source, including notation-like comments.
    const payload = value ?? stripNotationComments(fallback ?? "")
    if (!payload) return
  ...
  return (
    <Button
      ...
      aria-label={
        copyFailed ? "Copy failed" : copied ? (labels?.copied ?? "Copied") : (labels?.copy ?? "Copy code")
      }
      variant={variant}
      size={resolvedSize}
      onClick={handleClick}
