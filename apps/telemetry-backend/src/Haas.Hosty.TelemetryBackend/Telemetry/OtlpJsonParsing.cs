...
// Shared low-level readers for the OpenTelemetry collector `file` exporter's OTLP/JSON output, used by
// both the logs and the traces parser (the two signals share the OTLP resource/attribute/AnyValue
// encoding; only the record shapes differ). All readers are tolerant and never throw — a missing or
// malformed field yields its neutral value so one bad record cannot poison a tail. Copied from Core.
internal static class OtlpJsonParsing
{
    ...
    // OTLP timestamps are int64 nanoseconds, encoded as a JSON string in protojson (but tolerate a raw
    // number). False when the property is absent, malformed, or non-positive.
