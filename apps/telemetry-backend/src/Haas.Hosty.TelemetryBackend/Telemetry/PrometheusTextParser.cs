...
// One parsed Prometheus exposition sample: the metric name, its label set, and its value. Any trailing
// scrape timestamp is dropped — the backend stamps samples with the scrape time so every series in a
// tick shares one clock (and producers often omit it anyway).
...
// `prometheus` exporter serves at /metrics. Hand-written (no regex) so it stays AOT-clean and cheap to
// run every scrape tick: it works over ReadOnlySpan<char> and only allocates the strings it actually
// keeps (metric name and label key/value). Tolerant by design: a malformed line is skipped, never
// thrown, so one bad series cannot poison a whole scrape. Copied from Core (Phase 2).
internal static class PrometheusTextParser
{
    ...
    // Parses a double-quoted label value starting at the opening quote, honoring the Prometheus escapes
