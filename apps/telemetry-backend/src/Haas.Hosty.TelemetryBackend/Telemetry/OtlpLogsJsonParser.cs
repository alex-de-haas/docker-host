...
// attributed to via its `hosty.app.id` resource attribute. Records that carry no app id are dropped by
// the parser, never surfaced here.
...
// OTLP/JSON, one ExportLogsServiceRequest-shaped object (`{"resourceLogs":[...]}`) per line. Walks the
// tree with JsonDocument — AOT-safe (no reflection) — and never throws: a malformed line or a record
// missing its app attribution is skipped so one bad line cannot poison a tail. Missing record
// timestamps fall back to `fallbackTimestamp` (the tail clock) so unstamped records still land. Copied
// from Core (Phase 2).
internal static class OtlpLogsJsonParser
{
    ...
    // Prefers timeUnixNano, falls back to observedTimeUnixNano, then the tail clock.
