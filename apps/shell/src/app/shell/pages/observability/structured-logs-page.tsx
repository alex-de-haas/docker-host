...
import { OtlpLogTable, SEVERITY_FILTERS } from "../../observability/otlp-log-table";
import type { CoreApp, FleetOtlpLogsResponse } from "../../types";
...
export function ObservabilityStructuredLogsPage({
  runtimeApps,
  systemApps,
  coreOrigin,
}: {
  ...
  const loadLogs = useCallback(
    async (appId: string, severity: number, range: number, text: string) => {
      ...
      try {
        ...
        const response = await fetch(`${coreOrigin}/api/observability/logs?${params.toString()}`, {
          credentials: "include",
        });
