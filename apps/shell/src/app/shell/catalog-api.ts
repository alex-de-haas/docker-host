...
export async function getCatalogApps(coreOrigin: string): Promise<CatalogAppsResponse> {
  const response = await fetch(`${coreOrigin}/api/catalog/apps`, { credentials: "include" });
...
export async function getCatalogApp(coreOrigin: string, appId: string): Promise<CatalogAppDetail> {
  const response = await fetch(`${coreOrigin}/api/catalog/apps/${encodeURIComponent(appId)}`, { credentials: "include" });
  redirectToCoreLoginIfAuthRequired(response, coreOrigin);
  if (!response.ok) {
    throw new Error(await readCoreError(response));
  }
