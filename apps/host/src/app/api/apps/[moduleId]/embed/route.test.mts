...
test('GET /api/apps/[moduleId]/embed rejects unauthenticated callers', async t => {
  ...
  const response = await GET(
    new Request('http://localhost:3000/api/apps/com.example.reports/embed?path=%2F'),
    { params: Promise.resolve({ moduleId: 'com.example.reports' }) }
  );
...
async function createRouteTestConfig(): Promise<HostRuntimeConfig> {
  const dataRootContainer = await fs.mkdtemp(path.join(os.tmpdir(), 'docker-host-apps-embed-route-'));
