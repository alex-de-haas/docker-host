...
internal sealed partial class AppsCommand(CommandContext context)
{
    ...
    private async Task<int> BackupAsync(string[] args)
    {
        ...
        // then restarts it. Warn first so the short downtime is not a surprise.
    ...
    private static SourceOverrideOptions ParseSourceOverrideOptions(string[] args, HostyEnvironment environment)
    {
        if (args.Length == 0)
        {
