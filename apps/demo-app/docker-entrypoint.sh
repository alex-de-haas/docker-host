...
# then drop privileges with gosu before exec'ing the server. Only the `backend` service mounts the
# volume; for `frontend` this just creates an unused local directory, which is harmless.
