...
The Host must provide a Web UI as the primary management surface. Administrators use it to see system apps, runtime apps, Docker container status, settings, storage mounts, updates, app data backups, and new app installation.

Production-like Host launch is container-first:

- the Hosty Core/Shell package is currently distributed and run as a Docker image/container;
- initial launch and lifecycle management for the Host itself are handled by the standalone `hosty` CLI executable;
- `docker-host` remains a deprecated compatibility alias;
- the Web UI is the default Hosty Shell and management surface;
- the CLI can perform app/module operations, but only through the same Host backend API as the Web UI;
- app/module installation, update, and backup business logic lives in the Host backend and is not duplicated in the CLI.

The standalone `hosty` CLI is the reliable recovery path for Host container lifecycle operations: install, start, stop, restart, update, status, logs, open, and configuration. The module-management runtime works through the Host backend after the Host container is running, except for recovery flows that must work when the Host API is unavailable.

## Components

```mermaid
flowchart LR
  A["hosty CLI"] --> B["Docker daemon"]
  B --> C["Host container"]
  D["Browser"] --> E["Hosty Shell"]
  E --> F["Hosty Core API"]
  C --> E
  C --> F
  A --> F
  F --> B
  F --> G["Managed runtime app containers"]
```

### Hosty Core container

The Host container currently runs both Hosty Shell and Hosty Core API. The backend API contains app/module-management logic and talks to the Docker daemon through the mounted Docker socket.

The Host container is responsible for:

- installing runtime apps from manifest URLs and legacy modules from metadata URLs;
- runtime app container lifecycle;
- Docker daemon status for runtime apps;
- settings and storage mappings;
- app data backups and restore;
- app image updates;
...
### `hosty` CLI executable

The CLI is primarily for bootstrap and lifecycle management of the Host container itself.

The `hosty` CLI must be distributed as a standalone executable without an external runtime. The baseline implementation is a .NET self-contained single-file application using Spectre.Console for terminal UI, prompts, status output, tables, progress indicators, and command structure.
...
Lifecycle commands work directly through the Docker daemon because the Host API may not be running yet or may be broken. `hosty stop` also reads the Host data-root module registry and stops known module containers before stopping the Host container.

Auth bootstrap and recovery commands also remain local-first. `hosty auth setup-token` writes a one-time setup token hash into the Host data root so the first administrator can be created through `/setup` without relying on a pre-existing Host API session.

`hosty uninstall` preserves the CLI executable itself but removes Host-managed runtime and local state: the Host container, known module containers, Host/module images when Docker allows it, the shared module network when it is no longer in use, launch configuration, and Host state under the data root. After that, `hosty install` must recreate launch configuration and the baseline directory structure.
...
The CLI also provides app-management commands:
...
Fast terminal installation uses the Unix `scripts/install.sh`. This is a pure shell bootstrap script that downloads the latest development CLI executable from the `cli-dev` GitHub Release, installs it locally as `hosty`, keeps a deprecated `docker-host` alias, and prepares the first Host container launch.
...
- delegate local Docker endpoint and Linux-container-mode checks to the installed `hosty` CLI;
- detect OS/architecture;
- download the matching standalone CLI executable artifact from the `cli-dev` GitHub Release;
- verify `SHA256SUMS` when the checksum file is available and fail with a clear error if verification cannot be completed;
- place the preferred executable in a user-writable bin directory, such as `~/.hosty/bin/hosty`;
- copy a deprecated compatibility alias, such as `~/.hosty/bin/docker-host`;
- make the file executable;
- add `~/.hosty/bin` to the shell profile for future terminal sessions, or print the exact command if the profile cannot be detected;
- call `hosty install` to create the default Host data root `~/.hosty` and prepare Host container launch configuration in `~/.hosty/config/launch.env`;
- preserve existing `launch.env` values when the installer runs again;
- support scoped overrides for forks, installer tests, and custom shell profiles: `HOSTY_INSTALL_REPO`, `HOSTY_INSTALL_TAG`, `HOSTY_INSTALL_DIR`, `HOSTY_INSTALL_PROFILE`, `HOSTY_INSTALL_SKIP_PATH_UPDATE`, `HOSTY_INSTALL_START`;
- keep legacy `DOCKER_HOST_INSTALL_*` override names as compatibility aliases;
- avoid duplicating module-management logic;
- print next commands and the Web UI URL after installation.

`install.sh` must remain a shell-only bootstrap layer for Unix-like systems. The `hosty` CLI itself is not a shell script: it is a standalone executable that does not require an installed .NET runtime, Node.js/npm, or another package manager.
...
- published command name `hosty`, with a deprecated `docker-host` alias while compatibility is needed;
...
Native Windows support targets Docker Desktop with the WSL 2 Linux engine. Windows containers mode is explicitly unsupported. If Docker reports `OSType != linux`, `hosty install/start/status` should fail with a clear diagnostic that Docker Host requires Docker Desktop Linux containers.
...
CLI access to the Docker daemon is performed directly through the Docker Engine API over a local Unix socket or Windows named pipe. The Docker CLI executable is not a runtime dependency for the `hosty` CLI.
...
On native Windows, `hosty install` should persist a Windows-appropriate default:
...
`HOST_DATA_ROOT_HOST` should also be resolved to a platform-native absolute path during install, for example `C:\Users\<user>\.hosty` on Windows.
...
`hosty config` must be a typed interface to known Host launch settings, not an arbitrary editor for `launch.env`.
...
hosty config set HOST_DATA_ROOT_HOST ~/.hosty-dev
hosty config set HOST_IMAGE=docker-host:dev
...
```sh
export PATH="$HOME/.hosty/bin:$PATH"
```

`HOSTY_INSTALL_PROFILE` can explicitly set the profile path for custom shell setups. Legacy `DOCKER_HOST_INSTALL_PROFILE` remains accepted.
...
```sh
HOSTY_INSTALL_START=1 curl -fsSL https://raw.githubusercontent.com/alex-de-haas/docker-host/main/scripts/install.sh | sh
```

With `--start` or `HOSTY_INSTALL_START=1`, the script runs:
...
`install.sh` must stay a thin bootstrap layer. After installation, all further operations are handled by the standalone `hosty` executable.
...
`hosty install` must check local Docker Engine reachability, verify that the daemon is running in Linux-container mode, and prepare launch configuration:
...
- Host image reference: default value bundled with the CLI, overrideable through `hosty config`;
- Host data root: default `~/.hosty` on the administrator's machine, with `~/.docker-host` fallback for existing installs;
- Host container data mount: `<host-data-root>:/data`;
- Host container env: `HOST_DATA_ROOT_HOST=<host-data-root>` and `HOST_DATA_ROOT_CONTAINER=/data`;
- UI port mapping: CLI chooses a free host port by default, overrideable through `hosty config`;
- restart policy: default `unless-stopped`, overrideable through `hosty config`;
- container name: default `docker-host`, overrideable through `hosty config`;
...
-v ~/.hosty:/data
-e HOST_DATA_ROOT_HOST=~/.hosty
-e HOST_DATA_ROOT_CONTAINER=/data
--network <shared-module-network>
<host-image-reference>
```

All values except the container-side data root `/data` must be overrideable through `hosty config`.

`hosty start` creates or starts the Host container using the saved configuration.

`hosty open` opens the Web UI in a browser or prints the URL.
...
`hosty restart` must restart the Host container without changing app/module data.
...
- if the artifact differs, safely replace the installed `hosty` binary by downloading to a temporary file next to the target executable, setting permissions, and then replacing the target;
- refresh the deprecated `docker-host` alias from the same executable while compatibility is supported;
...
- recommend restarting the Host with `hosty stop` and `hosty start`;
- show a clear error if the CLI artifact update fails.

`hosty start` owns Host image refresh. When starting a stopped or missing Host container, it checks the configured image reference, pulls registry-backed references, falls back to a locally cached image when the pull fails, and recreates the Host container when the configured tag points at a different local image id. Running Host containers are not changed by `hosty start`; administrators can stop and start the Host when they want to adopt the newly pulled image.
...
Hosty Shell, Hosty Core API, Host Docker image definition and `hosty` CLI should live in one monorepo, while being released as separate artifacts. The detailed repository layout, GitHub Actions path filters, and versioning model are described in [Repository and release model](repository-release-model.md).
