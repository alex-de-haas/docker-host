...
The script is intentionally thin. It delegates launch configuration creation and Docker preflight to `docker-host install`, which owns `launch.env` parsing and validation. Re-running the installer is a repair/reinstall path: it may replace the CLI executable, but it preserves existing launch settings through the CLI config flow. The installer supports scoped overrides for forks, tests, custom shell profiles, and explicit start mode: `DOCKER_HOST_INSTALL_REPO`, `DOCKER_HOST_INSTALL_TAG`, `DOCKER_HOST_INSTALL_DIR`, `DOCKER_HOST_INSTALL_PROFILE`, `DOCKER_HOST_INSTALL_SKIP_PATH_UPDATE`, and `DOCKER_HOST_INSTALL_START`.
