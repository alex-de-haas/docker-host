...
The skill also distinguishes the Host API origin from the Host lifecycle mode. Agents should configure `HOST_DEV_REPOSITORY_PATH` for a source-run Host, or use `docker-host dev up --host-url` when validating modules against an already running Host process instead of the installed Host container.
