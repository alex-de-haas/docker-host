...
| --- | --- | --- | --- | --- |
| `image` | Compiled OCI image | OCI registry | Locked: tag → digest; a reviewed update advances the digest (the `rolling` per-start re-resolve was removed) | Digest, in Docker store |
...
3. The commit lock for OFF (pinned-commit checkout). The reviewed-update-advances-the-commit half is now part of the update flow itself (2026-07-22): the update plan resolves the manifest ref with a single `git ls-remote` — no clone, no fetch, so the periodic update sweep stays cheap and never materializes checkouts for apps that were never started — and surfaces the movement as a `source:{current}->{new}` change entry (the source counterpart of the image-digest delta). Apply stamps exactly the reviewed commit into `AppSourceState.Commit`, and the checkout catches up at the next start via `EnsurePinnedCommitAsync`'s fetch-and-retry. This closes the "ghost version" defect where the manifest advanced but the pin (and the running code) stayed behind. Local ref resolution (source-resolve, pin re-resolve) prefers `refs/remotes/origin/{branch}`, since `git fetch` never advances the clone-time local branch.
...
4. **`development: false` source = commit lock, reviewed update — full stop.** A build-to-production source runtime pins its commit and advances via reviewed update. A ref-re-fetch-every-start (`rolling`) mode was considered and later removed platform-wide: every artifact advance goes through a reviewed update, so start-time drift cannot bypass the review boundary.
...
- ~~`rolling` mode for a source build-to-prod runtime~~ — removed platform-wide (decision 4).
