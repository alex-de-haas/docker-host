...
3. The commit lock for OFF (pinned-commit checkout). The reviewed-update-advances-the-commit half is now part of the update flow itself (2026-07-22): the update plan resolves the manifest ref against the fetched managed checkout and surfaces the movement as a `source:{current}->{new}` change entry (the source counterpart of the image-digest delta), and apply stamps exactly the reviewed commit into `AppSourceState.Commit` — closing the "ghost version" defect where the manifest advanced but the pin (and the running code) stayed behind. Branch refs resolve against `refs/remotes/origin/{branch}` first, since `git fetch` never advances the clone-time local branch.
