...
- **Phase 1a — declared `development` flag + Source tab.** Add `development` to the runtime profile, validated source-only and **at most one per manifest** (`app_manifest_multiple_development_runtimes`), and surface it on `AppRuntimeProfileSummary`. Narrow `IsLiveSourceApp`/`AppRecord.Live` and `SupportsSource` to require a `development: true` runtime; add `development: true` to the in-repo `hosty.shell` / `demo-app` `dev` profiles (decision 3). Refine the Source settings tab: hide when there is no development runtime; target the single development runtime; edit its override without switching runtimes (keeps the existing single app-level override). Small, high-value, mostly independent of storage: fixes today's mislabeling of a build-to-production source runtime as Live/overridable, and unblocks "same source, two runtimes (release vs dev)". A `development: false` source runtime gets a commit lock + reviewed update.
