...
8. Ship the operator milestone — the `hosty.ai-gateway` system app (admin-only operator sessions on a host-resident CLI harness) plus the Shell assistant UI. Extracted to its own feature: [ai-gateway](../ai-gateway/plan.md); its deliverables live there, not here.
