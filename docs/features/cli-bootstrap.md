...
When the CLI runs inside WSL with Docker Desktop for Windows, Docker Desktop WSL integration must be enabled for the active distro. If integration is disabled, the Linux CLI cannot reach `/var/run/docker.sock` even when Docker Desktop itself is running.
