...
- **How the token reaches the page.** The page posts `hosty:request-delegated-token` to its parent
  ...
  would hand a user-scoped credential to whatever an operator installed. The page caches the token
  until it is nearly spent and drops it on a 401, so a save is not a fresh trip to Core. Standalone
  there is no embedder and therefore no token: the page says so at once rather than rendering an
...
- The settings page's inline script is compiled (not run) by the gateway suite, because it lives in
  ...
  embedder — the page's own request answered, the reply rendered, and the following save reusing the
  cached token instead of asking again.
