...
- **Authenticate the delegated token, not an app session.** Callers send
  `Authorization: Bearer <delegated token>`. Validate it locally with the SDK
  (`validateDelegatedToken` / `HostyDelegatedToken.Validate`), which checks it against
  `HOSTY_DELEGATED_TOKEN_PUBLIC_KEY` and the app id. The identity scheme in front of the app's other
  routes validates app identity tokens and refuses every delegated one.
...
- **Declare `annotations.readOnlyHint` on every tool.** `hosty mcp` and the gateway's external MCP
  endpoint are fail-closed: they export only tools with `readOnlyHint: true`, so a missing annotation
  hides the tool rather than widening access.
- **Authorize per tool for the delegated actor**, with the same permission model the app's HTTP
